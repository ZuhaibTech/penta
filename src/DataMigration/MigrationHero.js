...
export default function MigrationHero() {
  ...
  useGSAP(() => {
    ...
    gsap.set(".gsap-migration-video", { x: 50, opacity: 0, scale: 0.95 });

    // Animation Sequence
    tl.to(".gsap-migration-video", {
      x: 0,
      opacity: 1,
      scale: 1,
      duration: 1.4,
    })
    .to(".gsap-migration-text", {
      x: 0,
      opacity: 1,
      duration: 1,
      stagger: 0.2,
      ...
      <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row items-center gap-10 md:gap-16">
        ...
        <div className="gsap-migration-video flex-1 w-full order-2 md:order-2 px-2 md:px-0">
          <div className="relative mx-auto max-w-[550px] md:max-w-none group">
            {/* Glass Shell */}
            <div className="relative rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-[0_25px_50px_-12px_rgba(2,132,199,0.2)] border border-white/70 bg-white/20 backdrop-blur-md p-1 md:p-1.5">
        ...
        <div className="flex-1 space-y-6 md:space-y-10 text-center md:text-left order-3 md:order-1 px-2">
          <div className="space-y-4">
            <h1 className="gsap-migration-text hidden md:block text-5xl md:text-7xl font-black leading-[1.1]">
              Data <br /> 
              <span className="text-[#0284c7]">Migration</span>
            </h1>
            
            <p className="gsap-migration-text text-base md:text-xl text-slate-600 leading-relaxed max-w-xl mx-auto md:mx-0 font-medium">
