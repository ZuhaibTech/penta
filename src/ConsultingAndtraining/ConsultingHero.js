...
export default function ConsultingHero() {
  ...
  useGSAP(() => {
    ...
    // Initial State: Consistent with the hero design system
    gsap.set(".gsap-consult-text", { x: -50, opacity: 0 });
    gsap.set(".gsap-consult-video", { x: 50, opacity: 0, scale: 0.95 });

    // Animation Sequence
    tl.to(".gsap-consult-video", {
      x: 0,
      opacity: 1,
      scale: 1,
      duration: 1.4,
    })
    .to(".gsap-consult-text", {
      x: 0,
      opacity: 1,
      duration: 1,
      stagger: 0.2,
  ...
  return (
    <section 
      ...
      translate="no"
    >
      {/* Background Glass Glows - Provides depth for glassmorphism */}
      ...
      <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row items-center gap-8 md:gap-16">
        ...
        <div className="gsap-consult-video flex-1 w-full order-2 md:order-2 group">
          <div className="relative w-full h-[280px] md:h-[480px] rounded-[2.5rem] overflow-hidden shadow-[0_25px_50px_-12px_rgba(2,132,199,0.2)] border border-white/70 bg-white/20 backdrop-blur-md p-1 md:p-1.5">
            ...
            {/* Subtle Gradient for Refraction Look */}
            <div className="absolute inset-0 bg-gradient-to-br from-[#0284c7]/10 via-transparent to-white/10 pointer-events-none" />
          </div>
        </div>
        ...
        <div className="flex-1 space-y-6 md:space-y-10 text-center md:text-left order-3 md:order-1">
          <div className="space-y-4">
            <h1 className="gsap-consult-text hidden md:block text-5xl md:text-7xl font-black leading-[1.1] text-slate-900">
              Consulting And <br /> 
              <span className="text-[#0284c7]">Training</span>
            </h1>
            
            <p className="gsap-consult-text text-base md:text-xl text-slate-600 leading-relaxed max-w-xl mx-auto md:mx-0 font-medium">
